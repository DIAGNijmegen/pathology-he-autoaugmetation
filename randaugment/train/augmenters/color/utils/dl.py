...
from evaluation.metrics import accuracy_fn, spearman_fn
...
from utils.sigint import KilledException

try:
    # Prevent tf from blocking the entire GPU
    import tensorflow as tf
    config = tf.ConfigProto()
    config.gpu_options.allow_growth = True
    session = tf.Session(config=config)
except Exception as e:
    print('Failed to import and configure tensorflow: %s' % str(e), flush=True)
...
def fit_model(train_gen, val_gen, model, n_epochs, output_dir, train_step_multiplier=1.0, workers=1,
    ...
    if exists(join(output_dir, 'last_epoch.h5')) and exists(join(output_dir, 'history.csv')) and allow_resume_training:
        ...
        df = pd.DataFrame.from_csv(join(output_dir, 'history.csv'))
        epochs_run = len(df)
...
class HistoryCsv(callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if exists(self.file_path):
            df = pd.DataFrame.from_csv(self.file_path)
...
def plot_history(history_path, loss_list, metric_list, plot_path):
    ...
    # Format data
    df = pd.DataFrame.from_csv(history_path)
...
class FindBestEval(callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        try:
            # Read history
            df = pd.DataFrame.from_csv(self.history_path)
...
class EvalCustomMetric(callbacks.Callback):
    ...
    def update_log(self, val):
        ...
        # Read log
        df = pd.DataFrame.from_csv(self.log_path)
        ...
        for tag, metric_value in val.items():
            ...
            # Add latest value
            df.ix[len(df) - 1, tag] = metric_value
...
class CommonMetric(callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        # Compute new metric
        df.ix[len(df) - 1, self.new_metric] = self.fn_metric(df.ix[len(df) - 1, self.metric_tags])
...
class Normalize(layers.Layer):
    ...
    def __init__(self, normalize_dim=1, **kwargs):
    ...
    def call(self, inputs):
        ...
        # Normalize norm
        # Warning: be careful with axis, recommended to avoid when more than 2 dimensions (batch + 1).
        x = K.l2_normalize(inputs, axis=self.normalize_dim)

        # Avoid small numbers
        n = K.int_shape(inputs)[self.normalize_dim]
        n = K.sqrt(K.cast(n, 'float32'))
        x = x * n
...
class PenalizeCorrelation(layers.Layer):

    def __init__(self, factor=1e-3, normalize_axis=1, **kwargs):
        self.factor = factor
        self.normalize_axis = normalize_axis
        super(PenalizeCorrelation, self).__init__(**kwargs)
    ...
    def call(self, inputs):
        ...
        # Avoid small numbers
        n = K.int_shape(inputs)[self.normalize_axis]
        n = K.sqrt(K.cast(n, 'float32'))

        if channels == 2:
            ch1 = x[:, :, 0] / n  # normalization multiplied by n, here we remove it
            ch2 = x[:, :, 1] / n
            loss = K.mean(K.abs(K.batch_dot(ch1, ch2, axes=1)), axis=0)
            ...
            ch2 = x[:, :, 1] / n
            ch3 = x[:, :, 2] / n
            loss1 = K.mean(K.abs(K.batch_dot(ch1, ch2, axes=1)), axis=0)
            loss2 = K.mean(K.abs(K.batch_dot(ch1, ch3, axes=1)), axis=0)
            loss3 = K.mean(K.abs(K.batch_dot(ch2, ch3, axes=1)), axis=0)
        ...
        # else:
            ...
            #         if i > j:
            #             losses.append(K.mean(K.abs(K.batch_dot(K.l2_normalize(x[:, :, i], axis=self.normalize_axis), K.l2_normalize(x[:, :, j], axis=self.normalize_axis), axes=1)), axis=0))
            # losses = K.concatenate(losses, axis=0)
            # loss = K.mean(losses, axis=0)
...
class RGBtoHE_notlearned(layers.Layer):

    def __init__(self, **kwargs):
        super(RGBtoHE_notlearned, self).__init__(**kwargs)

    def call(self, inputs):
        x, kernel = inputs

        x = K.permute_dimensions(x, (0, 2, 1))  # required for batch_dot
        kernel = K.permute_dimensions(kernel, (0, 2, 1))  # required for batch_dot
        x = K.batch_dot(x, kernel, axes=(1, 2))
...
class RGBtoHE1(layers.Layer):

    def __init__(self, output_channels=1, **kwargs):
        self.output_channels = output_channels
        super(RGBtoHE1, self).__init__(**kwargs)

    def build(self, input_shape):
        # Create a trainable weight variable for this layer.
        self.kernel = self.add_weight(name='kernel',
                                      shape=(3, self.output_channels),
                                      initializer='uniform',
                                      trainable=True)
        self.bias = self.add_weight(shape=(self.output_channels,),
                                    initializer='uniform',
                                    name='bias')
        super(RGBtoHE1, self).build(input_shape)  # Be sure to call this at the end
    ...
    def compute_output_shape(self, input_shape):
        return (input_shape[0], input_shape[1], self.output_channels)


class RGBtoHE2(layers.Layer):

    def __init__(self, output_channels=2, **kwargs):
        self.output_channels = output_channels
        super(RGBtoHE2, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        self.bias = self.add_weight(shape=(self.output_channels,),
                                    initializer='uniform',
                                    name='bias')
        super(RGBtoHE2, self).build(input_shape)  # Be sure to call this at the end
...
class RGBtoHE3(layers.Layer):

    def __init__(self, output_channels=3, **kwargs):
        self.output_channels = output_channels
        super(RGBtoHE3, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        self.bias = self.add_weight(shape=(self.output_channels,),
                                    initializer='uniform',
                                    name='bias')
        super(RGBtoHE3, self).build(input_shape)  # Be sure to call this at the end
...
class HEtoRGB1(layers.Layer):

    def __init__(self, input_channels=1, clip_output=True, **kwargs):
        self.input_channels = input_channels
        self.clip_output = clip_output
        super(HEtoRGB1, self).__init__(**kwargs)

    def build(self, input_shape):
        # Create a trainable weight variable for this layer.
        self.kernel = self.add_weight(name='kernel',
                                      shape=(self.input_channels, 3),
                                      initializer='uniform',
                                      trainable=True)
        self.bias = self.add_weight(shape=(3,),
                                    initializer='uniform',
                                    name='bias')
        super(HEtoRGB1, self).build(input_shape)  # Be sure to call this at the end
...
class HEtoRGB2(layers.Layer):

    def __init__(self, input_channels=2, clip_output=True, **kwargs):
        self.input_channels = input_channels
        self.clip_output = clip_output
        super(HEtoRGB2, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        self.bias = self.add_weight(shape=(3,),
                                    initializer='uniform',
                                    name='bias')
        super(HEtoRGB2, self).build(input_shape)  # Be sure to call this at the end
...
class HEtoRGB3(layers.Layer):

    def __init__(self, input_channels=3, clip_output=True, **kwargs):
        self.input_channels = input_channels
        self.clip_output = clip_output
        super(HEtoRGB3, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        self.bias = self.add_weight(shape=(3,),
                                    initializer='uniform',
                                    name='bias')
        super(HEtoRGB3, self).build(input_shape)  # Be sure to call this at the end
...
def unet_network(x_input, n_channels, n_filters, l2_factor, activation='tanh'):

    conv1 = conv_op(x_input, n_filters[0], stride=2, bn=True, activation='lrelu', l2_factor=l2_factor)  # 32, 64, 64
    conv2 = conv_op(conv1, n_filters[1], stride=2, bn=True, activation='lrelu', l2_factor=l2_factor)  # 64, 32, 32
    conv3 = conv_op(conv2, n_filters[2], stride=2, bn=True, activation='lrelu', l2_factor=l2_factor)  # 128, 16, 16
    conv4 = conv_op(conv3, n_filters[3], stride=2, bn=True, activation='lrelu', l2_factor=l2_factor)  # 256, 8, 8
    conv5 = conv_op(conv4, n_filters[4], stride=2, bn=True, activation='lrelu', l2_factor=l2_factor)  # 512, 4, 4

    upconv_1 = up_conv_op(conv5, n_filters[3], bn=True, activation='lrelu', l2_factor=l2_factor)  # 256, 8, 8

    concat_2 = layers.concatenate([conv4, upconv_1], axis=-1)  # 256+256, 8, 8
    upconv_2 = up_conv_op(concat_2, n_filters[2], bn=True, activation='lrelu', l2_factor=l2_factor)  # 128, 16, 16

    concat_3 = layers.concatenate([conv3, upconv_2], axis=-1)  # 128+128, 16, 16
    upconv_3 = up_conv_op(concat_3, n_filters[1], bn=True, activation='lrelu', l2_factor=l2_factor)  # 64, 32, 32

    concat_4 = layers.concatenate([conv2, upconv_3], axis=-1)  # 64+64, 32, 32
    upconv_4 = up_conv_op(concat_4, n_filters[0], bn=True, activation='lrelu', l2_factor=l2_factor)  # 32, 64, 64

    concat_5 = layers.concatenate([conv1, upconv_4], axis=-1)  # 32+32, 64, 64
    network = up_conv_op(concat_5, n_channels, bn=False, activation=activation, l2_factor=l2_factor)  # 3, 128, 128
...
# if __name__ == '__main__':
    ...
    # evaluate_spearman(
    #     input_path=r"Y:\projects\BreastCancerPredictionWSI\journal2\models\combined3\bigan\exp_id_1\wsi_classifier\t16\0\separable_conv_slim_reg_0.2_1e-5\regular\validation\best\model_group_pred.csv",
    #     output_dir=r'Y:\projects\BreastCancerPredictionWSI\journal2\models\combined3\bigan\exp_id_1\wsi_classifier\t16\0\separable_conv_slim_reg_0.2_1e-5\regular\validation\best'
    # )
    #
    # from glob import glob
    # from os.path import basename, join, dirname
    #
    # for path in glob(r"Y:\projects\BreastCancerPredictionWSI\journal\models\combined3\bigan\exp_id_1\dense_1_128\model_pred.csv"):
    # # for path in glob(r"Y:\projects\BreastCancerPredictionWSI\journal\models\combined3\baseline\rgb_mean\wsi_classifier\c16\*\separable_conv_slim_reg_0.2_1e-5\regular\test\*\model_group_pred_macro.csv"):
    #     print(path)
    ...
    #         labels=labels,
    #         preds=preds,
    #         output_path=join(dirname(path), 'acc.csv')
    #     )
    #     # compute_roc_from_file_to_file(
    #     #     input_path=path,
    #     #     output_path=join(dirname(path), 'roc_macro.csv'),
    #     #     label_column='macro'
    #     # )