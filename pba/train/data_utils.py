...
import augmentation_transforms as augmentation_transforms_autoaug
...
class DataSet(object):
    ...
    def parse_policy(self, hparams):
        ...
        if hparams.use_hp_policy:
            ...
            if isinstance(raw_policy[0], list):
                ...
                tf.logging.info('using HP policy schedule, last: ')
                ...
                self.policy.extend(
                    parse_policy(raw_policy[split:],
                                 self.augmentation_transforms))
                tf.logging.info('using HP Policy, policy: ')
    ...
    def load_tiger(self, hparams):
        train_labels = []
        test_labels = []
        
        self.x_path=hparams.data_dir+'training_x'+hparams.train_set
        self.y_path=hparams.data_dir+'training_y'+hparams.train_set

        self.val_x_path = hparams.data_dir+hparams.val_set+'x.npy'
        self.val_y_path = hparams.data_dir+hparams.val_set+'y.npy'

        self.test_images, self.test_labels = np.load(self.val_x_path), np.load(self.val_y_path)
        if len(np.unique(self.test_labels))>2:
            print('loaded data has', len(np.unique(self.test_labels)),'classes')
            self.test_labels[self.test_labels==0 ]=0
            self.test_labels[self.test_labels==1 ]=1
            self.test_labels[self.test_labels==2 ]=0
            self.test_labels[self.test_labels==3 ]=1
            self.test_labels[self.test_labels==4 ]=1
            self.test_labels[self.test_labels==5 ]=1
            self.test_labels[self.test_labels==6 ]=1

            print('modified to ', len(np.unique(self.test_labels)),'classes')
        tf.logging.info('Loaded test set.')
        self.val_images = np.load(self.val_x_path)
        self.val_labels = np.load(self.val_y_path)
        if len(np.unique(self.val_labels))>2:
            print('loaded data has', len(np.unique(self.val_labels)),'classes')
            self.val_labels[self.val_labels==0 ]=0
            self.val_labels[self.val_labels==1 ]=1
            self.val_labels[self.val_labels==2 ]=0
            self.val_labels[self.val_labels==3 ]=1
            self.val_labels[self.val_labels==4 ]=1
            self.val_labels[self.val_labels==5 ]=1
            self.val_labels[self.val_labels==6 ]=1

            print('modified to ', len(np.unique(self.val_labels)),'classes')
        tf.logging.info('Loaded val set.')
        self.train_images = np.load(self.x_path)
        self.train_labels = np.load(self.y_path)
        
        tf.logging.info('Loaded train set')
        
        self.num_classes = 2  #len(np.unique(train_labels))
        tf.logging.info("Len train: {nc}".format(nc = self.num_classes))
        tf.logging.info("Len train: {shape}".format(shape=np.shape(self.train_images)))
        if len(np.unique(self.train_labels))>2:
            print('loaded data has', len(np.unique(self.train_labels)),'classes')
            self.train_labels[self.train_labels==0 ]=0
            self.train_labels[self.train_labels==1 ]=1
            self.train_labels[self.train_labels==2 ]=0
            self.train_labels[self.train_labels==3 ]=1
            self.train_labels[self.train_labels==4 ]=1
            self.train_labels[self.train_labels==5 ]=1
            self.train_labels[self.train_labels==6 ]=1

            print('modified to ', len(np.unique(self.train_labels)),'classes')
    def load_camelyon17(self, hparams):
        train_labels = []
        test_labels = []
    ...
    def load_data(self, hparams):
        ...
        elif hparams.dataset == 'camelyon17':
            self.load_camelyon17(hparams)

        elif hparams.dataset == 'tiger':
            self.load_tiger(hparams)
        else:
    ...
    def next_batch(self, iteration=None):
        ...
        #print('#############################Getting next mimibatch of augmented data')
        ...
        dset = self.hparams.dataset + '_' + str(len(self.train_images))
        ...
        for data in images:
            if not self.hparams.no_aug:
                if not self.hparams.use_hp_policy:
                    ...
                    #print('apply autoaugment policy')
