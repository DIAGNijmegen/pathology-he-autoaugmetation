...
class Model(object):
    ...
    def _build_graph(self, images, labels, mode):
        ...
        tf.logging.info("Loss: {cost}".format(cost = self.cost))
    ...
    def _build_train_op(self):
        ...
        optimizer = tf.train.AdamOptimizer(initial_lr)
...
class ModelTrainer(object):
    ...
    def extract_model_spec(self, checkpoint_path):
        ...
        tf.logging.warning(
            'Trying to load model from {}'.format(checkpoint_path))
    ...
    def eval_child_model(self, model, data_loader, mode):
        ...
        while True:
            try:
                accuracy, loss = helper_utils.eval_child_model(
    ...
    def _compute_final_accuracies(self, iteration):
        ...
        if (iteration >= self.hparams.num_epochs - 1):
            test_accuracy, _ = self.eval_child_model(self.meval, self.data_loader,
                                                  'test')
        else:
            test_accuracy = 0
    ...
    def run_model(self, epoch):
        ...
        if self.hparams.validation_size > 0:
            valid_accuracy, valid_loss = self.eval_child_model(self.meval,
                                                   self.data_loader, 'val')
        tf.logging.info('Train Acc: {}, Valid Acc: {}, Valid Loss: {}'.format(
            training_accuracy, valid_accuracy, valid_loss))
        utils.history_to_file(io_path = self.hparams.local_dir+self.hparams.val_set+self.hparams.ind+"_log.csv", 
        	epoch = epoch, lr = 0, train_obj = 0, train_metric = training_accuracy, 
        	valid_obj = valid_loss, valid_metric = valid_accuracy)
        utils.plot_history(history_path = self.hparams.local_dir+self.hparams.val_set+self.hparams.ind+"_log.csv", 
    		plot_path =self.hparams.local_dir+self.hparams.val_set+self.hparams.ind+"_log.png")
       

        return training_accuracy, valid_accuracy, valid_loss
...
def build_model(inputs, num_classes, is_training, hparams):
    ...
    Returns:
      The logits of the image model.
    """
    tf.logging.info('In build_model')
    scopes = setup_arg_scopes(is_training)
    if len(scopes) != 1:
        raise ValueError('Nested scopes depreciated in py3.')
    with scopes[0]:
...
class Model(object):
    ...
    def _setup_images_and_labels(self, dataset):
        """Sets up image and label placeholders for the model."""
        tf.logging.info('In _setup_images_and_labels')
        if dataset == 'cifar10' or dataset == 'cifar100' or self.mode == 'train':
            self.images = tf.placeholder(tf.float32,
                                         [self.batch_size, self.image_size, self.image_size, 3])
            self.labels = tf.placeholder(tf.float32,
    ...
    def _build_graph(self, images, labels, mode):
        ...
        Args:
          ...
          mode: string indicating training mode ( e.g., 'train', 'valid', 'test').
        """
        tf.logging.info('In _build_graph')
        is_training = 'train' in mode
        if is_training:
            self.global_step = tf.train.get_or_create_global_step()
    ...
    def _calc_num_trainable_params(self):
        tf.logging.info('In _calc_num_trainable_params')
        self.num_trainable_params = np.sum([
            np.prod(var.get_shape().as_list())
            for var in tf.trainable_variables()
        ])
    ...
    def _build_train_op(self):
        """Builds the train op for the model."""
        tf.logging.info('In _build_train_op')
        hparams = self.hparams
        tvars = tf.trainable_variables()
        grads = tf.gradients(self.cost, tvars)
        if hparams.gradient_clipping_by_global_norm > 0.0:
...
class ModelTrainer(object):
    ...
    def __init__(self, hparams):
        ...
        self._new_session()
        self._session.__enter__()
        tf.logging.info('In ModelTrainer init')
    ...
    def extract_model_spec(self, checkpoint_path):
        """Loads a checkpoint with the architecture structure stored in the name."""
        tf.logging.info('In extract_model_spec')
        tf.logging.warning(
            'Trying to load model from {}'.format(checkpoint_path))
        #ckpt = tf.train.get_checkpoint_state(os.path.dirname(checkpoint_path))
        self.saver.restore(self.session, checkpoint_path)
    ...
    def eval_child_model(self, model, data_loader, mode):
        ...
        Returns:
          Accuracy of the model on the specified dataset.
        """
        tf.logging.info('In eval_child_model')
        tf.logging.info('Evaluating child model in mode {}'.format(mode))
        while True:
            try:
    ...
    def _build_models(self):
        ...
        # Determine if we should build the train and eval model. When using
        # distributed training we only want to build one or the other and not both.
        tf.logging.info('In _build_models')
        with tf.variable_scope('model', use_resource=False):
            m = Model(self.hparams, self.data_loader.num_classes, self.data_loader.image_size)
            m.build('train')
            tf.logging.info("___________________________________________________________________________")
            tf.logging.info('model{}'.format(m))
            tf.logging.info("___________________________________________________________________________")
            self._num_trainable_params = m.num_trainable_params
            self._saver = m.saver
        with tf.variable_scope('model', reuse=True, use_resource=False):
    ...
    def _run_training_loop(self, curr_epoch):
        """Trains the model `m` for one epoch."""
        tf.logging.info('In _run_training_loop')
        start_time = time.time()
        while True:
            try:
                train_accuracy = helper_utils.run_epoch_training(
    ...
    def _compute_final_accuracies(self, iteration):
        """Run once training is finished to compute final test accuracy."""
        tf.logging.info('In _compute_final_accuracies')
        if (iteration >= self.hparams.num_epochs - 1):
