...
def _bottleneck_block_v1(inputs, filters, training, projection_shortcut,
  ...
  """
    ...
    if projection_shortcut is not None:
        ...
        shortcut = batch_norm(
            inputs=shortcut, training=training, data_format=data_format)
        tf.logging.info('Projection shortcut -> Batch norm')
    ...
    inputs = batch_norm(inputs, training, data_format)
    inputs = tf.nn.relu(inputs)
    tf.logging.info('2Dconv 1x1 -> Batch norm -> Relu')
    ...
    inputs = batch_norm(inputs, training, data_format)
    inputs = tf.nn.relu(inputs)
    tf.logging.info('2Dconv 3x3 -> Batch norm -> Relu')
    ...
    inputs += shortcut
    inputs = tf.nn.relu(inputs)
    tf.logging.info('2Dconv 1x1 -> Batch norm -> Shortcut -> Relu')
...
def block_layer(inputs, filters, bottleneck, block_fn, blocks, strides,
  ...
  """
    ...
    for k in range(1, blocks):
        tf.logging.info(
            "Block #{k}, number filters {f}".format(
            k = block_fn, f= filters_out))
...
def build_resnet_model(inputs, num_classes, hparams, training):
  ...
  """
    ...
    if resnet_version == 1:
        inputs = batch_norm(inputs, training, data_format)
        inputs = tf.nn.relu(inputs)
        tf.logging.info('First layer: inputs->batch_norm()->relu')
    ...
    for i, num_blocks in enumerate(block_sizes):
        ...
        tf.logging.info(
            "Layer number {i}, number of filters {nf}".format(i = i, nf = num_filters))
        tf.logging.info(
            "Block function {bfn}, number of blocks {nb}, blocks strides{bstr}".format(
            bfn = block_fn, nb= num_blocks,bstr = block_strides[i] ))
    ...
    inputs = tf.layers.Dropout(0.5)(inputs)
