...
#----------------------------------------------------------------------------------------------------
def log_sample_pred_label_to_file(io_path, prediction, label, dataset):
    if os.path.isfile(io_path): 
        df = pd.read_csv(io_path)
        #print('file exists')
    else:
        df = pd.DataFrame(columns=['dataset','prediction','label'])
        #print('creating')
    #print(df)   
    #print('lr',lr,'train_obj',train_obj,'train_metric',train_metric,'valid_obj',valid_obj)
    df = df.append({'dataset':[dataset*len(prediction)],'prediction': list(prediction),'label': list(label)}, ignore_index=True).to_csv(io_path,index=False)
#----------------------------------------------------------------------------------------------------
