...
'''
./c-submit --require-gpu-mem="8G" --gpu-count="1" --require-mem="20G" --require-cpus="8" --priority="high" --node="dlc-ditto" khrystynafaryna 9548 10 doduo1.umcn.nl/khrystynafaryna/kf_container_invariant:latest python3 /data/pathology/projects/autoaugmentation/from_chansey_review/manual/train.py --dataroot '/data/pathology/projects/autoaugmentation/from_chansey_upd/data/midog/patches/' --dataset midog --train_set training_hamamatsu_xr_ --val_set  validation_hamamatsu_s360_ 
...
parser.add_argument('--dataset', type=str, default='tiger',choices=['camelyon17','none','tiger',"midog"],
...
parser.add_argument('--save', type=str, default='/data/pathology/projects/autoaugmentation/from_chansey_review/manual/experiments/', help='experiment name')
...
parser.add_argument('--randaugment_transforms_set', type=str, default='hed_only',choices=['review','midl2021','original','midl2021_tr2eb','midl2021_trsh2eb','manual','hed_only'],help='which set of randaugment transforms to use')
...
project_name='manual'+args.val_set[3:-1]
...
def main():
  ...
  if args.randaugment_transforms_set=="manual":
    train_queue, valid_queue = get_dataloaders(args.dataset, args.batch_size, args.num_workers, dataroot=args.dataroot, train_set=args.train_set, val_set=args.val_set, augmenter='hed-light')
    print("Using augmentation hed-light")
  else:
    train_queue, valid_queue = get_dataloaders(args.dataset, args.batch_size, args.num_workers, dataroot=args.dataroot, train_set=args.train_set, val_set=args.val_set, augmenter='hed_only')
    print("Using augmentation hed only")
